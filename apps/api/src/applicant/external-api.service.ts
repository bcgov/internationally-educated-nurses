  ...
  */
  async filterContradictoryRows(applicants:AtsApplicant[]){
    ...
    const filteredApplicants =  applicants.filter((applicant)=>{
      const contradiction = existingApplicants.find(existingApplicant=>existingApplicant.ats1_id===applicant.ats1_id.toString() && existingApplicant.id !== applicant.applicant_id);
      if(contradiction){
