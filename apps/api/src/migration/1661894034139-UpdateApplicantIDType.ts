...
export class UpdateApplicantIDType1661894034139 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        // Need to clear 
        await queryRunner.connection.createQueryBuilder().update(IENApplicant).set({status:undefined}).execute();
        // Applicant status audits will need to be deleted because they will be referencing status ids that do not exist 
        await queryRunner.connection.createQueryBuilder().delete().from(IENApplicantStatusAudit).execute();
        ...
        await queryRunner.changeColumn('ien_applicant_status','id', new TableColumn({
            type:'uuid',
            name:'id',
            isNullable:false,
        }) )
        ...
        await queryRunner.changeColumn('ien_applicant_status_audit','applicant_id',new TableColumn({
            type:'uuid',
            name:'applicant_id',
            isNullable:true
        }) ) 
